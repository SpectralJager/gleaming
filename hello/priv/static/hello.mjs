...
// build/dev/javascript/hello/hello.mjs
var Model2 = class extends CustomType {
  constructor(counter, joke) {
    super();
...
var Rst = class extends CustomType {
};
var GetJoke = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(_) {
  return [new Model2(0, ""), none()];
}
function view(model) {
  let count = to_string(model.counter);
  return div(
    toList([class$("flex flex-col gap-4 items-center")]),
    toList([
      h1(
        toList([class$("text-5xl font-medium")]),
        toList([text("Press the cat!")])
      ),
      figure(
        toList([]),
        toList([
          ...
          figcaption(
            toList([class$("text-center text-xl")]),
            toList([text("cat clicked " + count + " times!")])
          )
        ])
      ),
      button(
        toList([on_click(new Rst())]),
        toList([text("reset counter")])
      ),
      p(
        toList([class$("px-8 text-center")]),
        toList([text(model.joke)])
      )
    ])
  );
}
...
function update(model, msg) {
  if (msg instanceof Inc) {
    return [model.withFields({ counter: model.counter + 1 }), get_fact()];
  } else if (msg instanceof Rst) {
    return [new Model2(0, ""), none()];
  } else if (msg instanceof GetJoke && msg[0].isOk()) {
    let joke = msg[0][0];
    return [model.withFields({ joke }), none()];
  } else {
    return [model, none()];
  }
}
